...
def plot_icp(prev_pcd, cur_pcd, transform=None, point_size=1.0):
    # Transform the previous point cloud using the transformation transform
    if transform is not None:
        prev_pcd.transform(transform)
    ...
    vis.create_window(window_name="After ICP Alignment")
    vis.add_geometry(prev_pcd)
    vis.add_geometry(cur_pcd)
